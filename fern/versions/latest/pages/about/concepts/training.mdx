...
This turns an environment into a synthetic data generation (SDG) pipeline. See [Offline Training (SFT/DPO)](/latest/training-tutorials/offline-training-w-rollouts) for a hands-on walkthrough.

## Next Steps

- **Try it**: [Training Tutorials](/latest/training-tutorials) for hands-on guides.
- **Related concept**: [Agent Harness Optimization](/latest/about/concepts/agent-harness-optimization) for improving agent performance without training.